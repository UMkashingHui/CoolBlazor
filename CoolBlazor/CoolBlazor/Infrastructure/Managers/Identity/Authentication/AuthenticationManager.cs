...
using CoolBlazor.Infrastructure.Routes;
    ...
    public class AuthenticationManager : IAuthenticationManager
    {
        ...
        public async Task<IResult> Login(TokenRequest model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                var tokenStorage = await _localStorage.GetItemAsync<string>(StorageConstants.Local.AuthToken);
                var getRefreshToken = await _localStorage.GetItemAsync<string>(StorageConstants.Local.RefreshToken);
                if (!string.IsNullOrEmpty(userImageURL))
                {
                    await _localStorage.SetItemAsync(StorageConstants.Local.UserImageURL, userImageURL);
                }

                await ((CoolBlazorStateProvider)this._authenticationStateProvider).StateChangedAsync();
        ...
        public async Task<IResult> Logout()
        {
            ...
            ((CoolBlazorStateProvider)_authenticationStateProvider).MarkUserAsLoggedOut();
