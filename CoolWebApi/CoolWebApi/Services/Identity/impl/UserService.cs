...
using CoolWebApi.Utils.Specifications;
...
namespace CoolWebApi.Services.Identity.impl
{
    public class UserService : IUserService
    {
        ...
        private async Task<string> SendVerificationEmail(CoolBlazorUser user, string origin)
        {
        ...
        {
            var user = _userManager.Users.Where(u => u.Id.ToString() == userId).FirstOrDefault();
        ...
        {
            var user = await _userManager.Users.Where(u => u.Id.ToString() == request.UserId).FirstOrDefaultAsync();
        ...
        public async Task<IResult> UpdateRolesAsync(UpdateUserRolesRequest request)
        {
            ...
            // if (user.Email == "kevinkashinghui@163.com")
            // {
            ...
            {
                var curUserHasAdministratorRole = selectedRoles
                ...
                if (!(curUserHasAdministratorRole && !userHasAdministratorRole) || !curUserHasAdministratorRole)
                {
                    return await Result.FailAsync(_localizer["Not Allowed to add or delete Administrator Role if you have no this role."]);
                }
                if (userHasAdministratorRole)
                {
                    return await Result.FailAsync(_localizer["Not Allowed to add or delete Administrator Role if this user already has Administrator Role."]);
                }
            }
        ...
        public async Task<string> ExportToExcelAsync(string searchString = "")
        {
            ...
            var result = await _excelService.ExportAsync(users, sheetName: _localizer["Users"],
                mappers: new Dictionary<string, Func<CoolBlazorUser, object>>
                {
